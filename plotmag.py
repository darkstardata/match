...
wdir = '/Users/andrew/git/match/science/uds/f160w/'
data = Table.read(wdir+'allsexmatch.tab', format='ascii.tab')
...
d = pixdia
l = (d-1)/2
segm = pyfits.open(wdir+'segm/segm_f160w.fits')[0].data
...
for z in xrange(xpix.size):
    ...
    for x in xrange(d):
        for y in xrange(d):
            length = np.sqrt(float(x-l)**2+float(y-l)**2)
            if length < l:
                xelem = int(xcen + x - l)
                if 0 <= xelem <= 962:
                    yelem = int(ycen + y - l)
...
binsig = np.asarray(np.where(np.logical_and(magdiffsex >= (mu-3*sigma), magdiffsex <= (mu+3*sigma))))[0]
...
ax1.set_xlabel(r'$\Delta m_{sim} (= m_{sim}-m)$', fontsize=15)
...
ax2.set_ylabel(r'$\Delta m_{sex} (=m_{sex}-m)$', fontsize=15)
